...
db_config = {
    ...
    'user': 'root', # Root para permisos de LOAD DATA
    'password': 'root', 
...
def generar_productos_csv(n_rows):
    ...
    with open(filename, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f, delimiter=',')
        for i in range(1, n_rows + 1):
            # Asumimos que existen 8 categorías (ids 1-8)
...
def run():
    ...
    print(f"⏱️ Tiempo Generación: {round(t_end_gen - t_start_gen, 2)} s")
    print(f"⏱️ Tiempo Ingesta: {round(t_end_load - t_start_load, 2)} s")
    print(f"⏱️ TIEMPO TOTAL: {round(time.time() - start_global, 2)} s")
    print("="*40)
